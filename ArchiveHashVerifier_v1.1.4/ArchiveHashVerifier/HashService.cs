...
public static partial class HashService
{
    ...
    {
        HashKind[] selectedKinds = kinds.Distinct().ToArray();
        var hashes = selectedKinds.Where(k => k != HashKind.Blake3)
            .ToDictionary(k => k, k => IncrementalHash.CreateHash(HashCatalog.Get(k).Algorithm!.Value));
        Hasher? blake3 = selectedKinds.Contains(HashKind.Blake3) ? Hasher.New() : null;
        ...
        try
        {
            ...
            progress?.Report(new(fileBytes, fileLength, overallBefore + fileBytes, overallLength, sourcePath));
            var values = hashes.ToDictionary(x => x.Key, x => Convert.ToHexString(x.Value.GetHashAndReset()).ToLowerInvariant());
            if (blake3 is not null)
                values.Add(HashKind.Blake3, Convert.ToHexString(blake3.Finalize().AsSpan()).ToLowerInvariant());
            return values;
        }
        finally
        {
