...
public sealed partial class ProcessingCoordinator
{
    ...
    public async Task<OperationSummary> GenerateAsync(
        ...
        CancellationToken cancellationToken, IProgress<OperationPhase>? phase = null, Func<BatchOutputNameRequest,CancellationToken,Task<string?>>? selectBatchOutputName = null)
    {
        if (options.BatchOutput) return await GenerateBatchAsync(sourceFiles, options, confirmOverwrite, progress, message, cancellationToken, phase, selectBatchOutputName).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken, IProgress<OperationPhase>? phase = null, Func<Task<bool>>? confirmMissingPublicKey = null)
    {
        ...
        bool IsManifest(string x) => BatchArtifacts.IsManifestPath(x);
        bool IsBatch(string x) { if(!HashCatalog.TryFromPath(x,out HashSpec? spec) || spec is null) return false; return BatchArtifacts.ClassifyChecksumFile(x,spec) is not ChecksumFileClassification.LegacySidecar; }
        foreach (string manifest in canonical.Where(IsManifest).Select(BatchArtifacts.ResolveManifestPath).Distinct(StringComparer.OrdinalIgnoreCase))
        ...
        IProgress<OperationPhase>? phase)
    {
        ...
        foreach (string sidecar in group.Sidecars.Where(x => HashCatalog.TryFromPath(x, out _)))
        {
            ...
            try { string? value; ChecksumFileClassification classification = BatchArtifacts.ClassifyChecksumFile(sidecar, spec!); if (classification == ChecksumFileClassification.MalformedStrictChecksumList) throw new InvalidDataException("一括Hash形式が不正です。"); if (classification == ChecksumFileClassification.StrictChecksumList) { if (!BatchArtifacts.TryReadEntry(sidecar, spec!, Path.GetFileName(group.SourcePath), out value)) continue; } else value = HashService.ReadExpectedHash(sidecar, spec!.Kind); parseable.Add((sidecar, spec!, value!)); }
    ...
    public static IReadOnlyList<VerificationGroup> BuildVerificationGroups(IReadOnlyCollection<string> inputs, OperationSummary summary)
    {
        ...
        foreach (string source in sources)
        {
            ...
            foreach (string batch in Directory.EnumerateFiles(Path.GetDirectoryName(source)!)) { if (!HashCatalog.TryFromPath(batch, out HashSpec? batchSpec) || batchSpec is null) continue; try { ChecksumFileClassification classification = BatchArtifacts.ClassifyChecksumFile(batch, batchSpec); if (classification == ChecksumFileClassification.MalformedStrictChecksumList) throw new InvalidDataException("一括Hash形式が不正です。"); if (classification == ChecksumFileClassification.StrictChecksumList && BatchArtifacts.TryReadEntry(batch, batchSpec, Path.GetFileName(source), out _)) sidecars.Add(batch); } catch (Exception ex) { summary.Results.Add(new(source, batchSpec.DisplayName, ResultState.Error, ex.Message)); } }
