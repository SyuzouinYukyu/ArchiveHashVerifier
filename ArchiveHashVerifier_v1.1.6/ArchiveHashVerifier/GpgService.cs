...
public sealed partial class GpgService
{
        ...
        bool autoVerify, CancellationToken cancellationToken, IProgress<OperationPhase>? phase = null)
    {
        ...
        if (!autoVerify)
        {
            ...
            string? signerUid = null;
            try
            {
                IReadOnlyList<GpgKey> keys = await ListUsableSecretKeysAsync(cancellationToken).ConfigureAwait(false);
                signerUid = keys.FirstOrDefault(x => x.Fingerprint.Equals(unverifiedFingerprint, StringComparison.OrdinalIgnoreCase) ||
                    x.SigningFingerprints.Any(f => f.Equals(unverifiedFingerprint, StringComparison.OrdinalIgnoreCase)))?.UserId;
            }
            catch (OperationCanceledException) { throw; }
            catch (Exception ex) { System.Diagnostics.Debug.WriteLine("GPG signer UID metadata lookup failed: " + ex.Message); }
            return new(GpgVerifyState.CreatedUnverified, "生成成功 / 自動検証未実施", signerUid,
