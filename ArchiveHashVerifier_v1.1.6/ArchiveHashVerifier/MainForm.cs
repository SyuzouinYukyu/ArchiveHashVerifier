...
{
    private const string AppTitle = "ArchiveHashVerifier v1.1.6";
    ...
    private readonly CheckBox blake3 = new() { Text = "BLAKE3", AutoSize = true, Checked = true };
    ...
    private readonly CheckBox batchOutput = new() { Text = "方式ごとに1ファイルへまとめて出力する", AutoSize = true, Checked = true };
    ...
    private void InitializeComponent()
    {
        ...
        modePanel.Controls.Add(batchOutput);
        verifyRadio.CheckedChanged += (_, _) => UpdateModeUi();
        generateRadio.CheckedChanged += (_, _) => UpdateModeUi();

        generationPanel.Controls.AddRange([sha512, sha256, sha3_512, sha3_256, blake3, pgpAsc, pgpSig, autoVerifyGpg, keyButton, keyLabel]);
    ...
    private void RestoreSettings()
    {
        ...
        blake3.Checked = settings.GenerateBlake3;
        ...
        batchOutput.Checked = settings.GenerateBatchOutput;
        UpdateKeyLabel();
    ...
    private async void StartButton_Click(object? sender, EventArgs e)
    {
        ...
        if (generateRadio.Checked && !sha512.Checked && !sha256.Checked && !sha3_512.Checked && !sha3_256.Checked && !blake3.Checked && !pgpAsc.Checked && !pgpSig.Checked)
        ...
        try
        {
            if (generation)
            {
                var options = new GenerationOptions
                {
                    ...
                    SigningFingerprint = settings.GpgFingerprint, BatchOutput = batchOutput.Checked
                };
                summary = await coordinator.GenerateAsync(files, options, ConfirmOverwriteAsync, progress, AppendResultThreadSafe, operationCts.Token, phase, SelectBatchOutputNameAsync);
            }
            else
            {
                summary = await coordinator.VerifyAsync(files, progress, AppendResultThreadSafe, operationCts.Token, phase, () => Task.FromResult(MessageBox.Show(this, "OpenPGP署名を検証するための公開鍵がありません。\r\n署名を未検証のままハッシュのみ検証しますか？", AppTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes));
            }
        }
    ...
    private Task<string?> SelectBatchOutputNameAsync(BatchOutputNameRequest request, CancellationToken cancellationToken)
    {
        if (InvokeRequired)
        {
            var tcs = new TaskCompletionSource<string?>(TaskCreationOptions.RunContinuationsAsynchronously);
            BeginInvoke(() => { try { tcs.SetResult(SelectBatchOutputNameCore(request)); } catch (Exception ex) { tcs.SetException(ex); } });
            cancellationToken.Register(() => tcs.TrySetCanceled(cancellationToken));
            return tcs.Task;
        }
        return Task.FromResult(SelectBatchOutputNameCore(request));
    }

    private string? SelectBatchOutputNameCore(BatchOutputNameRequest request)
    {
        while (true)
        {
            using var dialog = new SaveFileDialog
            {
                Title = "一括出力ファイル名を指定", InitialDirectory = request.Directory, FileName = request.InitialName,
                AddExtension = false, CheckFileExists = false, CheckPathExists = true, OverwritePrompt = false, ValidateNames = true
            };
            if (dialog.ShowDialog(this) != DialogResult.OK) return null;
            if (BatchArtifacts.TryGetBaseName(dialog.FileName, request.Directory, out _, out string error)) return dialog.FileName;
            MessageBox.Show(this, error, AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
    }
    private HashSet<HashKind> SelectedHashes()
    {
        ...
        if (blake3.Checked) selected.Add(HashKind.Blake3);
        return selected;
    }
    ...
    private void MainForm_FormClosing(object? sender, FormClosingEventArgs e)
    {
        ...
        settings.GenerateBlake3 = blake3.Checked;
        settings.AutoVerifyGeneratedSignatures = autoVerifyGpg.Checked;
        settings.GenerateBatchOutput = batchOutput.Checked;
        TrySaveSettings(showWarning: true);
    }
