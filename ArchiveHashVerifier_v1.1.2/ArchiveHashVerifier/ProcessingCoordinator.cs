...
public sealed class ProcessingCoordinator
{
    ...
    {
        var summary = new OperationSummary();
        IReadOnlyList<VerificationGroup> groups = BuildVerificationGroups(inputs, summary);
        long overallLength = SafeTotalLength(groups.Select(x => x.SourcePath));
        long overallBefore = 0;
        ...
        IProgress<OperationPhase>? phase)
    {
        ...
        foreach (string sidecar in group.Sidecars.Where(x => HashCatalog.TryFromPath(x, out _)))
        {
            ...
            try { parseable.Add((sidecar, spec!, HashService.ReadExpectedHash(sidecar, spec!.Kind))); }
